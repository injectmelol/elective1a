<?php
	...
	if(isset($_POST['facultyupdate']))
	{
		...
		$insertsql = "INSERT INTO Facultymember(personid,ssn,homestreetaddress,homecity,homezip,homephone,daytimephone,adjuncthiredate,fulltimehiredate,retiredate,emailaddress,dob) 
		values(?,?,?,?,?,?,?,?,?,?,?,?);";
		$statement = $db->prepare($insertsql);	
		...
		$id = $result['PersonId'];
		echo $id.'c';
		$updatesql = "UPDATE Faculty SET ssn = ?,homestreetaddress = ?,homecity = ?,homezip = ?,homephone = ?,daytimephone = ?, adjuncthiredate = ?,fulltimehiredate = ?,retiredate = ?,emailaddress = ?,dobwhere = ? where PersonId = ?";
		$statement = $db->prepare($updatesql);
